...
- **Asystent głosowy:** `lib/screens/assistant.dart` — `speech_to_text` + `flutter_tts` + `permission_handler`, wybór języka, POST `/api/voice`. Backend: jeśli ustawiony `GEMINI_API_KEY` (env), `/api/voice` używa **Gemini** (`gemini-2.0-flash`, przez `urllib`, `gemini_intent()`) do zrozumienia komendy → akcja add_task/complete_task; bez klucza fallback na słowa kluczowe pl/uk/en. Emulator zwykle nie ma rozpoznawania mowy → działa pole tekstowe.
