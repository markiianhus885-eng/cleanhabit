...
- **Asystent głosowy:** `lib/screens/assistant.dart` — `speech_to_text` + `flutter_tts` + `permission_handler`, wybór języka, POST `/api/voice`. Backend: jeśli ustawiony `ANTHROPIC_API_KEY` (env), `/api/voice` używa **Claude** (SDK `anthropic`, `claude_intent()`, model z `CLAUDE_MODEL`, domyślnie `claude-opus-4-8`) do zrozumienia komendy → akcja add_task/complete_task; bez klucza fallback na słowa kluczowe pl/uk/en. Emulator zwykle nie ma rozpoznawania mowy → działa pole tekstowe.
